...
// the octi CLI (free). When "true", the legacy Anthropic HTTP path is
// used as a fallback for environments that still have API budget.
...
// (default, free) or the Anthropic HTTPS API (fallback, paid). Selection
// is controlled by SENTINEL_INSIGHTS_USE_API=true. We keep the API path
// around so operators with budget can still opt in, but the default is
// cost-free dispatch through octi.
...
// callLLMViaAPI sends a prompt to the Anthropic Messages API.
