...
func (g *Generator) gatherInputs(ctx context.Context) (*GeneratorInputs, error) {
	...
	inputs.GovernanceDenyCounts = gatherGovernanceDenies(ctx, g.pool)
...
func (g *Generator) detectSignals(inputs *GeneratorInputs) []InsightCategory {
	...
	// Pattern: failures exist, OR any governance deny_reason has spiked
	// above 10 in the last 24h. The governance branch is what turns free
	// CLI dispatch into actual emitted insights — without it the signal
	// layer silently gates everything.
	totalFailures := 0
...
func (g *Generator) generateCategory(ctx context.Context, cat InsightCategory, inputs *GeneratorInputs) ([]Insight, error) {
	...
	case CategoryPattern:
		// When the only signal is a governance deny spike, use the
		// governance-specific briefing; otherwise use the generic
		// failure-pattern briefing.
		hasFailures := false
		for _, c := range inputs.FailureCounts {
			if c > 0 {
				hasFailures = true
		...
		if !hasFailures && len(inputs.GovernanceDenyCounts) > 0 {
...
// last 24h. Accepts anything satisfying the pgxpool Query shape so the
// production pool and test fakes both work.
func gatherGovernanceDenies(ctx context.Context, pool *pgxpool.Pool) map[string]int {
